...
// project 3.0
  ...
  static purchase = async(req,res)=>{
    ...
    if(!user_data['BALANCE'] || user_data['BALANCE'] == 'undefined'){
      ...
      if(user_balance >= item_price){
        ...
        let data = {...body , finished : false , DATE : date , TIME : time , ITEM_ID : item_id , INV : INV , REDEEM_DATE : date};
    ...
    if(!body || !body['item_id']){
      ...
      if(item_purchased){
        ...
        if(days > parseInt(item_purchased['validity']) ){
          ...
          if( parseInt(item_purchased['validity']) - days > 0){
            ...
            await Purchase.findOneAndUpdate({ITEM_ID : item_purchased['ITEM_ID']} , {
              $inc : {
                ...
                daily_income : item_purchased['daily_income']
              },
            ...
            if(this_user_parent !== 'undefined' && this_user_parent !== 0){
                  ...
                  let level3_rebade =
                    (parseFloat(item_purchased['daily_income'])/100)*3;
          ...
          return res.send({status : 'everything was done'})
        }
  ...
  static deposited = async (req,res)=>{
    ...
    console.log(amount , transaction_id);
    if(!trans_id_exist){
